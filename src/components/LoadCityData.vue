...
<template>
  <div class="flex">
    ...
    <div class="w-2/5 bg-[#133267] p-4">
      ...
      <img class="w-[150px] h-auto mt-6"
           :src="`http://openweathermap.org/img/wn/${data.current.weather[0].icon}@2x.png`" alt=""/>
      <div class="flex justify-between">
        <h1 class="text-7xl text-white">{{ Math.round(data?.current.temp) }} &deg; C</h1>
        <div>
          <h4 class="text-2xl text-white mt-6">Ressenti {{ Math.round(data?.current.feels_like) }} &deg;C</h4>
        ...
        <div class="max-h-[200px] overflow-y-auto mt-2">
          ...
          <div v-for="hourData in data.hourly" :key="hourData.dt" class="flex justify-between">
            ...
            <img class="w-auto h-[50px] object-cover"
                 :src="`http://openweathermap.org/img/wn/${hourData.weather[0].icon}.png`" alt=""/>
      ...
      <div class="text-white mt-8">
        ...
        <div class="max-h-[300px] overflow-y-auto mt-6">
          ...
          <div v-for="day in data.daily" :key="day.dt"
               class="flex items-center border-lg border-[#2B487C] rounded-b shadow sm:rounded-lg bg-[#2B487C] p-4 mt-2">
